...
_Note_: All examples written in this README use the ES6 specification.
...
A _collection_ is an API endpoint for a list of entities, for instance `http://api.example.com/articles`. Create it using the `all(name)` syntax:
...
A _member_ is an API endpoint for a single entity, for instance `http://api.example.com/articles/1`. Create it using the `one(name, id)` syntax:
...
Once you have collections and members endpoints, fetch them to get _entities_. Restful.js exposes `get()` and `getAll()` methods for fetching endpoints. Since these methods are asynchronous, they return a native Promise for response.
...
_Tip_: You can describe a member based on a collection _and_ trigger the API fetch at the same time by calling `get(id)`:
