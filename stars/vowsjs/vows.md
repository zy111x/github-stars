...
A batch can be either _synchronous_ or _asynchronous_. For a synchronous batch, the `topic` function just returns a value, and the test functions measure that value:
...
Note that if a batch's `topic` returns more than one value to its callback, they will be provided _in order_ for any sub-batches' `topic`, but hierarchically _in reverse order_. This may be a little confusing.
...
./node\_modules/.bin/vows test/\*.js
...
./node\_modules/.bin/vows test/\*.js test/\*.coffee
...
Test-driven development means roughly that write your tests _first_, then write the implementations, then keep running the tests till they work.
...
DEBUG=vows:\* ./node\_modules/.bin/vows mytest.js
