...
**_cortex-a57_** or newer CPU architecture on **Linux**.

All Apple M chips on **macOS**.

### `armv7`

**_cortex-a7_** or newer CPU architecture.
...
async function main() {
  ...
  await promises.writeFile(join(\_\_dirname, 'simple.png'), pngData)
}
...
GlobalFonts.registerFromPath(join(\_\_dirname, '..', 'fonts', 'AppleColorEmoji@2x.ttf'), 'Apple Emoji')
GlobalFonts.registerFromPath(join(\_\_dirname, '..', '\_\_test\_\_', 'fonts', 'COLRv1.ttf'), 'COLRv1')
...
writeFileSync(join(\_\_dirname, 'draw-emoji.png'), b)
...
`PathKit` is a toolset for manipulating Path in `Skia`, supporting **_quadratic beziers_**, **_cubic beziers_** and **_conics_**. The main features are.
...
$ sudo dnf install clang # https://fedora.pkgs.org/34/fedora-x86\_64/clang-12.0.0-0.3.rc1.fc34.x86\_64.rpm.html
$ yarn build
