...
_If you are an end user (i.e. a user of a tool that uses cosmiconfig, like `prettier` or `stylelint`), you can skip down to the end user section._
...
-   You (the cosmiconfig user) use cosmiconfig's asynchronous API.
-   Your end user runs a version of Node that supports ESM (\>=12.17.0, or earlier with the `--experimental-modules` flag).
-   Your end user provides an `.mjs` configuration file, or a `.js` file whose nearest parent `package.json` file contains `"type": "module"`. (See Node's method for determining a file's module system.)
...
$import: node\_modules/@foocorp/config/.prettierrc.base.yml
# we want more space!
printWidth: 200
