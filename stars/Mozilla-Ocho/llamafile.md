...
client \= OpenAI(
    base\_url\="http://localhost:8080/v1", \# "http://<Your api-server IP>:port"
    api\_key \= "sk-no-key-required"
)
...
Llama-3.2-3B-Instruct.Q6\_K.llamafile
...
Llama-3.2-1B-Instruct.Q6\_K.llamafile
...
gemma-2-27b-it.Q6\_K.llamafile
...
mistral-7b-instruct-v0.2.Q4\_0.llamafile
...
Phi-3-mini-4k-instruct.F16.llamafile
...
mixtral-8x7b-instruct-v0.1.Q5\_K\_M.llamafile
...
wizardcoder-python-34b-v1.0.Q5\_K\_M.llamafile
...
Meta-Llama-3-70B-Instruct.Q4\_0.llamafile
...
Meta-Llama-3-8B-Instruct.Q5\_K\_M.llamafile
...
e5-mistral-7b-instruct-Q5\_K\_M.llamafile
...
./mistral-7b-instruct-v0.2.Q5\_K\_M.llamafile --temp 0.7 -p '\[INST\]Write a story about llamas\[/INST\]'

And here is an example for WizardCoder-Python command-line llamafile:

./wizardcoder-python-13b.llamafile --temp 0 -e -r '\`\`\`\\n' -p '\`\`\`c\\nvoid \*memcpy\_sse2(char \*dst, const char \*src, size\_t size) {\\n'
...
5.  Finally, with the tools included in this project you can create your _own_ llamafiles, using any compatible model weights you want. You can then distribute these llamafiles to other people, who can easily make use of them regardless of what kind of computer they have.
...
curl -L -o mistral.gguf https://huggingface.co/TheBloke/Mistral-7B-Instruct-v0.1-GGUF/resolve/main/mistral-7b-instruct-v0.1.Q4\_K\_M.gguf
...
sudo sh -c "echo -1 > /proc/sys/fs/binfmt\_misc/WSLInterop"
...
\[1\] Darwin kernel versions 15.6+ _should_ be supported, but we currently have no way of testing that.
...
If you have both an AMD GPU _and_ an NVIDIA GPU in your machine, then you may need to qualify which one you want used, by passing either `--gpu amd` or `--gpu nvidia`.
...
llamafile \\
  -m wizardcoder-python-13b-v1.0.Q8\_0.gguf \\
  --temp 0 -r '}\\n' -r '\`\`\`\\n' \\
...
llamafile -ngl 9999 \\
  -m mistral-7b-instruct-v0.1.Q4\_K\_M.gguf \\
...
llamafile -m llama-65b-Q5\_K.gguf -p '
...
Researcher:' --interactive --color --batch\_size 1024 --ctx\_size 4096 \\
...
(
  ...
  echo '\[/INST\]'
      ...
      -m mistral-7b-instruct-v0.2.Q5\_K\_M.gguf \\
      -f /dev/stdin \\
      -c 0 \\
      --temp 0 \\
      -n 500 \\
...
llamafile -ngl 9999 --temp 0 \\
  ...
  -m llava-v1.5-7b-Q4\_K.gguf \\
  --mmproj llava-v1.5-7b-mmproj-Q4\_0.gguf \\
...
llamafile -ngl 9999 --temp 0 \\
    ...
    --no-display-prompt 2>/dev/null |
  sed -e's/ /\_/g' -e's/$/.jpg/'
a\_baby\_monkey\_on\_the\_back\_of\_a\_mother.jpg
...
llamafile -ngl 9999 \\
  -m llava-v1.5-7b-Q8\_0.gguf \\
  --mmproj llava-v1.5-7b-mmproj-Q8\_0.gguf \\
  --host 0.0.0.0
...
\-m
llava-v1.5-7b-Q8\_0.gguf
--mmproj
llava-v1.5-7b-mmproj-Q8\_0.gguf
--host
0.0.0.0
-ngl
9999
