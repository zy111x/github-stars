...
-   MistServer - Public domain streaming media server that works with any device and any format. (Source Code) `Unlicense` `C++`
...
_Related: Video Surveillance, Media Streaming - Multimedia Streaming_
...
-   Tube - Youtube-like (_without censorship and features you don't need!_) Video Sharing App written in Go which also supports automatic transcoding to MP4 H.265 AAC, multiple collections and RSS feed. (Demo) `MIT` `Go`
...
-   asciinema - Web app for hosting asciicasts. (Demo, Source Code) `Apache-2.0` `Elixir/Docker`
...
-   Geeftlist - Collaborative platform for managing, sharing and reserving gifts between friends and family. `GPL-3.0` `Docker`
...
_Related: Inventory Management, Resource Planning_
...
-   HedgeDoc - Realtime collaborative markdown notes on all platforms, formerly known as CodiMD and HackMD CE. (Demo, Source Code) `AGPL-3.0` `Docker/Nodejs`
...
-   SilverBullet - Note-taking application optimized for people with a hacker mindset. (Demo, Source Code, Clients) `MIT` `Docker/Deno`
...
-   lesma - Simple paste app friendly with browser and command line. (Demo, Source Code) `GPL-3.0` `Rust/Docker`
...
-   Glance - A highly customizable dashboard that puts all your feeds in one place. `AGPL-3.0` `Docker/Go`
...
_Related: Static Site Generators, Photo and Video Galleries, Content Management Systems (CMS)_
...
-   Immich Kiosk - A lightweight slideshow for running on kiosk devices and browsers that uses Immich as a data source. `GPL-3.0` `Docker/Go`
...
-   OpnForm - Beautiful open-source form builder. (Demo, Source Code) `AGPL-3.0` `PHP/Nodejs/Docker`
...
_Related: Money, Budgeting & Management, Inventory Management_
...
_Related: Software Development - Low Code_
...
_Related: Software Development - IDE & Tools_
...
-   Manifest - A complete backend that fits into 1 YAML file. (Demo, Source Code) `MIT` `Nodejs`
...
_Related: Blogging Platforms, Photo and Video Galleries, Content Management Systems (CMS)_
...
_Related: Media Streaming - Video Streaming_
...
-   Pomerium - An identity-aware reverse proxy, successor to now obsolete oauth\_proxy. It inserts an OAuth step before proxying your request to the backend, so that you can safely expose your self-hosted websites to public Internet. (Source Code) `Apache-2.0` `Go`
...
_Related: Note-taking & Editors, Static Site Generators, Knowledge Management Tools_

_See also: Wikimatrix, List of wiki software - Wikipedia, Comparison of wiki software - Wikipedia_
