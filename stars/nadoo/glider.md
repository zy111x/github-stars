...
linux redirect proxy

Redir6

√

linux redirect proxy(ipv6)

TProxy

√

linux tproxy(udp only)
...
OPTION:
  -check string
        ...
        check=http://HOST\[:PORT\]\[/URI\]\[#expect=REGEX\_MATCH\_IN\_RESP\_LINE\]
        check\=https://HOST\[:PORT\]\[/URI\]\[#expect=REGEX\_MATCH\_IN\_RESP\_LINE\]
        check\=file://SCRIPT\_PATH: run a check script, healthy when exitcode\=0, env vars: FORWARDER\_ADDR,FORWARDER\_URL
  ...
  \-checktolerance int
        fowarder check tolerance(ms), switch only when new\_latency < old\_latency - tolerance, only used in lha mode
  \-config string
        config file path
  \-dialtimeout int
  ...
  -service value
        run specified services, format: SERVICE\_NAME\[,SERVICE\_CONFIG\]
  -strategy string
        rr: Round Robin mode
...
--
Forwarder Options: FORWARD\_URL#OPTIONS
...
Services:
   dhcpd: service=dhcpd,INTERFACE,START\_IP,END\_IP,LEASE\_MINUTES\[,MAC\=IP,MAC\=IP...\]
          service=dhcpd-failover,INTERFACE,START\_IP,END\_IP,LEASE\_MINUTES\[,MAC\=IP,MAC\=IP...\]
...
SS scheme:
  ...
  Available methods for ss:
    AEAD Ciphers:
      AEAD\_AES\_128\_GCM AEAD\_AES\_192\_GCM AEAD\_AES\_256\_GCM AEAD\_CHACHA20\_POLY1305 AEAD\_XCHACHA20\_POLY1305
    Stream Ciphers:
      AES-128-CFB AES-128-CTR AES-192-CFB AES-192-CTR AES-256-CFB AES-256-CTR CHACHA20-IETF XCHACHA20 CHACHA20 RC4-MD5
    Alias:
          chacha20-ietf-poly1305 = AEAD\_CHACHA20\_POLY1305, xchacha20-ietf-poly1305 = AEAD\_XCHACHA20\_POLY1305
    Plain: NONE
...
--
SSR scheme:
  ssr://method:pass@host:port?protocol=xxx&protocol\_param=yyy&obfs=zzz&obfs\_param=xyz
...
glider -config CONFIG\_PATH

-   ConfigFile
    -   glider.conf.example
    -   office.rule.example
-   Examples
    -   transparent proxy with dnsmasq
    -   transparent proxy without dnsmasq

Service
-------

-   dhcpd / dhcpd-failover:
    -   service=dhcpd,INTERFACE,START\_IP,END\_IP,LEASE\_MINUTES\[,MAC=IP,MAC=IP...\]
...
-   systemd: https://github.com/nadoo/glider/tree/main/systemd
...
open \`feature.go\` & \`feature\_linux.go\`, comment out the packages you don't need
