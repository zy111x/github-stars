...
Constructor function that defines the layout (height / width) of each element
...
Constructor function the defines the data for each element
...
Used to maintain scroll position in case view gets destroyed, which often happens with back navigation

rowRenderer

Yes

(type: string | number, data: any, index: number) => JSX.Element | JSX.Element\[\] | null

Method that returns react component to be rendered. You get the type, data, index and extendedState of the view in the callback
...
Initial offset you want to start rendering from; This is very useful if you want to maintain scroll context across pages.
...
If true, the list will operate horizontally rather than vertically
...
On scroll callback function that executes as a user scrolls

onRecreate

No

(params: OnRecreateParams) => void

callback function that gets executed when recreating the recycler view from context provider
...
Use this to pass your on implementation of BaseScrollView
...
Callback function executed when the end of the view is hit (minus onEndThreshold if defined)
...
Specify how many pixels in advance for the onEndReached callback
...
Provides visible index; helpful in sending impression events
...
(Deprecated in 2.0 beta) Provides visible index; helpful in sending impression events
...
Specify the initial item index you want rendering to start from. Preferred over initialOffset if both specified
...
Default is false; if enabled dimensions provided in layout provider will not be strictly enforced. Use this if item dimensions cannot be accurately determined
...
Enables animating RecyclerListView item cells (shift, add, remove, etc)
...
A callback function that is executed when an item of the recyclerListView (at an index) has been layout. This can also be used as a proxy to itemsRendered kind of callbacks.
    ...
    refreshControl: (
      <RefreshControl
        ...
        onRefresh\={async () \=> {
          ...
          analytics.logEvent('Event\_Stagg\_pull\_to\_refresh');
          await refetchQueue();
