...
VertexAI parameter acquisition tutorial
...
config\_url=https://The address where you can access the download configuration/api.yaml
...
1.  Start with the remote api.yaml configuration file

docker run -d \\
  --name pro-api \\
  -e config\_url=http://你的服务器/api.yaml \\
  -e secret\_key=123456789 \\
  -p 8001:8000 \\
  -v $(pwd)/api.yaml:/app/api.yaml
-v $(pwd)/data:/app/data:rw
duolabmeng/pro-api:latest

config\_url automatically downloads the remote configuration file secret\_key encrypted with aes, ECB, 128 bits, if you want to be safe remember to enable the aes password, if you don't fill it in, you will get the plaintext configuration

1.  If you want to use Docker Compose

services:
  pro-api:
    container\_name: pro-api
    image: duolabmeng/pro-api:latest
    environment:
      - config\_url=http://file\_url/api.yaml
      - secret\_key=123456789
    ports:
      - 8001:8000
    volumes:
...
For example, if you are not in a position to modify the configuration file on a certain platform, you can upload the configuration file to a hosting service, which can provide a direct link for pro-api to download. config\_url is this direct link. If you don't want to restart the container to update the configuration, you can just refresh the configuration by accessing /reload\_config.
